...
export default function TraxNavigation() {
  ...
  return (
    <div className="size-full min-h-screen bg-black overflow-auto relative [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
      <div className="mx-auto w-full max-w-[430px] pb-[180px]">
        {currentScreen === "inicio" && <Container />}
        {currentScreen === "ventas" && <HtmlBody />}
        {currentScreen === "negocio" && <HtmlBodyCart />}
        {currentScreen === "crecer" && <HtmlBodyCart />}
      </div>
